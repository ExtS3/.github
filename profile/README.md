<div align="center">
  
<strong>2026-1 중부대학교 정보보안SW융합전공</strong>
<h1>Extemsion Supply chain Security System (ES3)</h1>

<img width="800" alt="ChatGPT Image 2026년 5월 19일 오후 02_25_06" src="https://github.com/user-attachments/assets/7019f606-7f63-471c-a4f7-79b0f27496fc" />

<br/>

<h2 align="center">Project Overview</h2>

<p align="center">
ES3는 비개발 프로그램의 공급망 보안 강화를 목표로 하는 보안 분석 시스템입니다.<br />
정적/난독화/동적 분석을 통해 프로그램 설치 전 위험 단서를 수집하고,<br />
악성 행위 가능성, 과도한 권한, 의심스러운 코드 패턴 등을 사전에 검토할 수 있도록 지원합니다.<br />
<br />
또한 Repository Manager와 연동하여 업데이트 기반 자동 검사와 레트로 분석을 수행하며,<br />
새로운 확장 프로그램 사용 요청 시 Holding 과정을 통해 평판 기반의 안정성을 함께 제공합니다.
</p>

<br />

<h2 align="center">Project Team</h2>

<table align="center">
<tr>
<td align="center" width="160">
<a href="https://github.com/Interludeal">
<img src="https://github.com/Interludeal.png" width="80" height="80" alt="Interludeal" />
</a>
<br />
<b>정재성</b>
<br />
<sub><b>PM</b></sub>
<br />
<sub>난독화/레트로 분석 로직</sub>
</td>

<td align="center" width="160">
<a href="https://github.com/J4EH00N">
<img src="https://github.com/J4EH00N.png" width="80" height="80" alt="J4EH00N" />
</a>
<br />
<b>심재훈</b>
<br />
<sub><b>PA</b></sub>
<br />
<sub>자동설치 및 정정 분석 로직</sub>
</td>

<td align="center" width="160">
<a href="https://github.com/Juhyeok0603">
<img src="https://github.com/Juhyeok0603.png" width="80" height="80" alt="Juhyeok0603" />
</a>
<br />
<b>윤주혁</b>
<br />
<sub><b>PA</b></sub>
<br />
<sub>웹 서버 프론트/백엔드 개발</sub>
</td>

<td align="center" width="160">
<a href="https://github.com/kkaturi14">
<img src="https://github.com/kkaturi14.png" width="80" height="80" alt="kkaturi14" />
</a>
<br />
<b>곽민경</b>
<br />
<sub><b>PA</b></sub>
<br />
<sub>시나리오 수집 및 Holding</sub>
</td>

<td align="center" width="160">
<a href="https://github.com/Pandyo">
<img src="https://github.com/Pandyo.png" width="80" height="80" alt="Pandyo" />
</a>
<br />
<b>박혜수</b>
<br />
<sub><b>PA</b></sub>
<br />
<sub>AI 기반 동적 분석 로직</sub>
</td>
</tr>
</table>

<br />

</div>
